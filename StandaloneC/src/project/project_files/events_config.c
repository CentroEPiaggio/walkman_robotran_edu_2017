...
#define MIN_SH_SAG  ( -10.0 * DEG_TO_RAD )
...
void events_sdl(Screen_sdl *screen_sdl, Simu_real_time *real_time, MBSdataStruct *MBSdata, int cur_t_usec)
{
	...
	if (cur_t_usec >= real_time->next_generic_keyboard_event_usec) 
	{
        ...
        // Key R: reset the floating base to the upright position:
        // This will only reset the robot body orientation
        else if (keystates[SDL_SCANCODE_R])
        {

         printf("Key R is pressed ... \n");
         wait_key_generic(real_time, cur_t_usec, 0.2);
         MBSdata->user_IO->restart=1;
