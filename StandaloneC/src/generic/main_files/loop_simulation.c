...
void loop_iteration(Loop_arguments *loop_arguments)
{
    ...
    #ifdef YARP
        updateDataFromYarp(loop_arguments->RobotranYarp_interface, MBSdata);
    #endif
    ...
    // update fixed joint forces (necessary for Force-Torque sensor)
    ...
    #ifdef YARP
        updateDataToYarp(loop_arguments->RobotranYarp_interface, MBSdata);
    #endif

    loop_arguments->x += h;
    // Resetting the floating base:
    if (MBSdata->user_IO->restart==1)
        {
            for (j=1;j<=6;j++)
            {
                vout[j]=MBSdata->user_IO->FB_state[j];
            }
            MBSdata->user_IO->restart=0;
        }
    // state update
