...
/**/	
	...
	gamma2 = lut->z[indx+1][indy]/((lut->x[indx+1]-lut->x[indx])*(lut->y[indy+1]-lut->y[indy]));
	zeta = lut->z[indx][indy+1]/((lut->x[indx+1]-lut->x[indx])*(lut->y[indy+1]-lut->y[indy]));
	
	z = alpha*((lut->x[indx+1]-x)*(lut->y[indy+1]-y))+beta*((x-lut->x[indx])*(y-lut->y[indy]))+gamma2*((x-lut->x[indx])*(lut->y[indy+1]-y))+zeta*((lut->x[indx+1]-x)*(y-lut->y[indy]));
...
double* polin1_manyOutput(LUT_2D *lut, double x, double* y)
{
	int indx;
	int step,min,max;
	double alpha;
	...
	for(i=1;i<=lut->ny ;i++){
		...
		alpha = (lut->z[indx+1][i]-lut->z[indx][i])/(lut->x[indx+1]-lut->x[indx]);
	
		y[i] = alpha*(x-lut->x[indx])+lut->z[indx][i];
...
/* Ajout par N. Docquier, 22/02/2007 */
...
LUT_1D* load_LUT_1D_file(char* fileName){
	...
	if(flut==0){
		sprintf(line, "Impossible to find the file %s.", fileName );
		mexErrMsgTxt(line);
	}

	fgets(line,256,flut);	// read the information line
	fgets(line,256, flut);  // read the number of elements
	...
	for(i=1;i<=lut->nx;i++)	{	
		fscanf(flut,"%lf %lf", &lut->x[i], &lut->y[i]);
		fgets(line,256,flut);
	}
