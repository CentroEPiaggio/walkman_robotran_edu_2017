...
*/
	...
	for(i=1;i<=nL;i++)
	{
		has_a_line_of_zeros = true;
		for(j=1;j<=nC;j++)
		{
			lds->Mr[i][j] = lds->Mruc[i][j]; // Muu
			if (lds->Mr[i][j]>1e-16)
				has_a_line_of_zeros = false;
		}
		if (has_a_line_of_zeros)
		{
			...
			for(k=1;k<=nC;k++)
				printf("lds->Mr[i][j] = %e;\n",i,k,lds->Mr[i][k]);
			mexErrMsgTxt("The reduced mass matrix has a line of zeros");
		}
